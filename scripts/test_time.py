...
#                 else:
#                     print("Something went wrong with the estimators")
...
#                                         add_information=f"{des}_RFECV_{estimator}",
#                                         modeltype=modeltype,
#                                         scoretype=score,
#                                     )
#                     x_train, x_test, y_train, y_test = ph.prepare_data(
#                                         score,
#                                         f"/data/shared/projects/pharmacophore_hot_spot_analysis/phantomdragon/data/slim/ref_set_{des}_RFECV_ki_pKd_pKi_pIC50_{estimator}.csv",
#                                         f"/data/shared/projects/pharmacophore_hot_spot_analysis/phantomdragon/data/slim/core_set_{des}_RFECV_ki_pKd_pKi_pIC50_{estimator}.csv",
        ...
        for k in datatypes:
            for des in descriptor:
                for t in test_val:
                    ...
                    tmp_timelist =[]
                    blub = ph.parameterCollector(
                                        add_information=f"{name}_{t}",
                                        modeltype=modeltype,
                                        scoretype=score,
                                    )
                    x_train, x_test, y_train, y_test = ph.prepare_data(
                                        score,
                                        f"/data/shared/projects/pharmacophore_hot_spot_analysis/phantomdragon/data/GRAIL-scores/PDBbind_refined_set_grail_scores_{des}.csv",
                                        f"/data/shared/projects/pharmacophore_hot_spot_analysis/phantomdragon/data/GRAIL-scores/PDBbind_{t}_set_grail_scores_{des}.csv",
                                        f"/data/shared/projects/pharmacophore_hot_spot_analysis/phantomdragon/data/experiments/refined_set.csv",
                                        f"/data/shared/projects/pharmacophore_hot_spot_analysis/phantomdragon/data/experiments/{t}_set.csv",
                                        f"{name}_{t}"
                                    )
                    blub.set_trainingdata(x_train,y_train)
                    blub.set_testingdata(x_test,y_test)
