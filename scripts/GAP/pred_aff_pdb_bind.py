...
def parseArguments():
    parser = argparse.ArgumentParser(description='Performs GRAIL affinity predictions for a set of input ligand-protein complexes.')
    ...
    parser.add_argument('-o',
                        dest='out_csv_file',
                        required=True,
                        help='[Required] The path of the output CSV-file containing the predicted affinity values calculated for each input complex.',
                        nargs=1)
...
def outputColNames(out_file):
    out_file.write('PDB code, pKi, pKd, pKi/pKd\n')
    out_file.flush()
