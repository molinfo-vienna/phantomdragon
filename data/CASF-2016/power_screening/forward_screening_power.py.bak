...
decoylst=filter(None,list(set(decoylst2)))
...
if fav == 'positive':
	...
	for i in targetlst:
		...
		for m in range(1,t10+1):
			forwardf.ix[tmp][m]=''.join(dfsorted[m-1:m].index.tolist())
		forwardf.ix[tmp]['Target']=i
		tmp+=1
		Topligand=cc.ix[i]['L1']
		tartemp=cc.ix[i]
