...
def usage():
        ...
        print("-l or --ligandinfo: specify the location of 'LigandInfo.dat' in the CASF-2016 package")
        print("-o or --output: input the prefix of output result files. Default is My_Reverse_Screening_Power")
...
try:
	options,args = getopt.getopt(sys.argv[1:],"hc:s:l:p:o:",["help","coreset=","score=","ligandinfo=","prefer=","output="])
...
if t1 < 1:
        print(("The number of top 1%% targets is %0.1f,less than 1"%(round(len(decoylst)*0.01))))
        print("In this case, we set the cutoff of top 1%% = 1")
        t1=1
if t5 < 1:
        print(("The number of top 5%% targets is %0.1f,less than 1"%(round(len(decoylst)*0.05))))
        print("In this case, we set the cutoff of top 5%% = 1")
        t5=1
if t10 < 1:
        print(("The number of top 10%% targets is %0.1f,less than 1"%(round(len(decoylst)*0.10))))
...
Ligdf=pd.DataFrame()
reversedf=pd.DataFrame(index=list(range(1,len(pdb)+1)),columns=list(range(0,t10+1)))
