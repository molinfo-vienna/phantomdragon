...
mkdir results_reverse_screening_negative
...
do
	...
	do
		...
		do
			...
			do
				python reverse_screening_power.py -c CoreSet.dat -s GAP_Scores/$data$model$add_info$score -l LigandInfo.dat -p 'negative' -o 'GAP-Score' > results_reverse_screening_negative/$data$model$add_info$score.out
