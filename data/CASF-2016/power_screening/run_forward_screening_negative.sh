...
mkdir results_forward_screening_negative
...
do
	...
	do
		...
		do
			...
			do
				python forward_screening_power.py -c CoreSet.dat -s GAP_Scores/$data$model$add_info$score -t TargetInfo.dat -p 'negative' -o 'GAP-Score' > results_forward_screening_negative/$data$model$add_info$score.out
